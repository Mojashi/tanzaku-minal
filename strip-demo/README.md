...
**どこまで入っているか**（実測 2026-08-09、49,954 ファイル / 101 万発言）:

- **ファイルは全部入っている。** ディスク上のセッションファイル数と索引済みファイル数が一致する
- **1 発言は先頭 1200 字まで**（`MAX_TEXT`）。全体の 22% がここで切られている。ツール結果は
  1 件が数 MB になることがあり、末尾は検索対象として意味を持たないため。長い発言の
  後半だけに出てくる語は引けない
- **再開位置は「行」で持つ。** かつてこれを「メッセージ数」で持っていて、
  行はメッセージだけではないので毎回**前回の末尾を読み直して二重に入れていた**。
  索引の 37% が同じ発言の写しだった（`--dedupe` で掃除できる）
